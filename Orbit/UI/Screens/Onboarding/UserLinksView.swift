...
struct UserLinksView: View {
    ...
    let linkOptions = ["Instagram", "X", "LinkedIn", "TikTok", "Bluesky", "Other"]

    var body: some View {
        Form {
            Section(header: Text("What are some links you'd like to share with other Astronauts?")) {
                ForEach(linkOptions, id: \.self) { option in
                    MultipleSelectionRow(title: option, isSelected: selectedLinks.contains(option)) {
            ...
            ToolbarItem(placement: .confirmationAction) {
                Button("Finish") {
                    Task {
                        ...
                        userVM.currentUser?.userLinks = selectedLinks.map { UserLinkModel(linkName: $0) }
