    ...
    init(user: UserModel) {
        ...
        if let dobString = user.dob {
            ...
            if let date = DateFormatterUtility.parseDateOnly(dobString) {
                print(
                    "Successfully parsed DOB using parseDateOnly: \(dobString) -> \(date)"
                )
                _dateOfBirth = State(initialValue: date)
            } else if let date = DateFormatterUtility.parseISO8601(dobString) {
                print(
                    "Successfully parsed DOB using parseISO8601: \(dobString) -> \(date)"
                )
                _dateOfBirth = State(initialValue: date)
            } else if let date = DateFormatterUtility.parseISODate(dobString) {
                print(
                    "Successfully parsed DOB using parseISODate: \(dobString) -> \(date)"
                )
                _dateOfBirth = State(initialValue: date)
            } else {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        if isFormValid {
                            ...
                            let formattedDate =
                                DateFormatterUtility.formatDateOnly(dateOfBirth)
