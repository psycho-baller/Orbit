...
struct LocationChoosingView: View {
    ...
    init(
        initialCoordinate: CLLocationCoordinate2D, pinLocation: Binding<CLLocationCoordinate2D>,
        onShareLocation: @escaping (CLLocationCoordinate2D) -> Void
    ) {
        self._cameraPosition = State(
            initialValue: .camera(MapCamera(centerCoordinate: initialCoordinate, distance: 500)))
